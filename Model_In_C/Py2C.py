...
class Py2C:
    ...
    def __init__(self, model_path = "best_loss_checkpoint.h5", torch = True, input_size = (1, 320), type="float", fxp_para=(21, 8),num_of_output=1, choose_only_output=False, ide="vs"):
        self.torch = torch
    ...
    def convert2C_func(self):
        ...
        for i in range(len(self.config["layers"])):
            ...
            if (layer.find("Add") >= 0) and (layer.find("Pad") < 0):
                ...
                if self.type == "fxp" and self.index_P2D == 0:
                    ...
                    self.fxp_inc = ""
                if hasattr(self.model.layers[i], "input_shape") and self.model.layers[i].input_shape is not None:
                    input_shapes = self.model.layers[i].input_shape
                else:
                    input_shapes = [tuple(t.shape) for t in self.model.layers[i].input]
                if len(input_shapes[0]) == 4:
                    NumOfInput = len(input_shapes)
                    source_add = ""
                    argument_add = ""
                    size = input_shapes[0][1] * input_shapes[0][2] * input_shapes[0][3]
                    for k in range(0, NumOfInput):
                        source_add += "input_" + str(k) + "[i]"
                        argument_add += str(self.type) + " input_" + str(k) + "[" + str(size) + "], "
                        if k < (NumOfInput - 1):
                    ...
                    source_add = ""
                    argument_add = ""
                    size = input_shapes[0][1] * input_shapes[0][2]
                    for k in range(0, NumOfInput):
                        source_add += "input_" + str(k) + "[i]"
                        argument_add += str(self.type) + " input_" + str(k) + "[" + str(size) + "], "
                        if k < (NumOfInput - 1):
                    ...
                    assert ((len(input_shapes[0]) == 3) or (len(input_shapes[0]) == 4)), "add layer hasn't supported 1 dimension yet"
            ...
            if layer.find("Concatenate") >= 0:
                ...
                if hasattr(self.model.layers[i], "input_shape") and self.model.layers[i].input_shape is not None:
                    ...
                    input_shapes = [tuple(t.shape) for t in self.model.layers[i].input]
                dimesion_input = len(input_shapes[0])
                axis = self.model.layers[i].axis
                assert (((dimesion_input == 3) and (axis == -1)) or ((dimesion_input == 4) and (axis == 3))) , "concatenate layer only supported 2 dimesion input combine with axis = -1 and 3 dimesion input combine with axis = 3 yet!!!"

                if hasattr(self.model.layers[i], "output_shape") and self.model.layers[i].output_shape is not None:
                    output_shape = self.model.layers[i].output_shape
                else:
                    output_shape = tuple(self.model.layers[i].output.shape)
                ...
                if len(input_shapes[0]) == 4:
                    ...
                    num_param = 0
                    for k in range(0, NumOfInput):
                        size = input_shapes[k][1] * input_shapes[k][2] * input_shapes[k][3]
                        source += "\tfor (int i = 0; i < " + str(size) + ";i++){\n\t\toutput[" + str(num_param) + " + i] = input_" + str(k) + "[i];\n\t}\n"
                        argument += str(self.type) + " input_" + str(k) + "[" + str(size) + "], "
                        num_param += size
                    self.call_function += "\t" + str(self.type) + " " + self.config["layers"][i]['config'][
                        'name'] + "[" + str(output_shape[1] * output_shape[2] * output_shape[3]) + "];\n"
                ...
                if len(input_shapes[0]) == 3:
                    ...
                    num_param = 0
                    for k in range(0, NumOfInput):
                        size = input_shapes[k][1] * input_shapes[k][2]
                        source += "\tfor (int i = 0; i < " + str(size) + ";i++){\n\t\toutput[" + str(num_param) + " + i] = input_" + str(k) + "[i];\n\t}\n"
                        argument += str(self.type) + " input_" + str(k) + "[" + str(size) + "], "
                        num_param += size
                    self.call_function += "\t" + str(self.type) + " " + self.config["layers"][i]['config']['name'] + "[" + str(output_shape[1] * output_shape[2]) + "];\n"
        ...
        ######################################### SET UP INPUT PHARSE ##################################################
        def _get_inbound_names(layer_config):
            inbound = layer_config.get('inbound_nodes', [])
            if not inbound:
                return []
            node = inbound[0]
            ...
            if 'OutModel' in Layer_name:
                if self.choose_only_output == True:
                    for i in range(len(self.config["layers"])):
                        if 'Dense' in self.config["layers"][i]['class_name']:
                            if self.config["layers"][i]['config']['activation'] == 'softmax' and self.count_dense_output == 0:
                                inbound = _get_inbound_names(self.config["layers"][i])
                                if inbound:
                                    input_name = inbound[0]
                                    self.count_dense_output += 1
                                    return input_name
                            if self.config["layers"][i]['config']['activation'] == 'softmax' and self.count_dense_output == 1:
                                inbound = _get_inbound_names(self.config["layers"][i+1])
                                if inbound:
                                    input_name = inbound[0]
                                    self.count_dense_output += 1
                                    return input_name
                            if self.config["layers"][i]['config']['activation'] == 'softmax' and self.count_dense_output == 2:
                                inbound = _get_inbound_names(self.config["layers"][i+2])
                                if inbound:
                                    input_name = inbound[0]
                                    self.count_dense_output += 1
                                    return input_name
        ...
        def find_input_for_add(Layer_name, index):
            ...
            for i in range(len(self.config["layers"])):
                if self.config["layers"][i]['config']['name'] == Layer_name:
                    inbound_names = _get_inbound_names(self.config["layers"][i])
                    for k in range(0, len(inbound_names)):
                        argu_add += inbound_names[k]
                        argu_add += ", "
            return argu_add
