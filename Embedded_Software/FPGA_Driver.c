...
/*  ... must match axi_if.v START_BASE_PHYS = 40'h00_A000_0000 */
#define REG_MMAP_SIZE	 0x0000000010000000LL
/*  ... 96MB covers A0..A5 windows used by RTL decode */
...
volatile struct CGRA_info {
  ...
  U32  *reg_mmap;     // user-virt mapped AXI register/LMM space
  U64  reg_mmap_size; // mapped size in bytes
...
static int is_target_dev(char *d_name, const char *target)
{
  ...
  strip_trailing_ws(name);
  strip_trailing_ws(target_buf);
  if (strcmp(name, target_buf) != 0) return 0;
  return 1;
}
...
int cgra_open()
{
  ...
  const char *UIO_AXI_CGRA      = "CGRA";
  const char *UIO_AXI_GENERIC   = "MY_IP";
  const char *UIO_DDR_HIGH      = "ddr_high";
  
  if ((num_dirs = scandir("/sys/class/uio", &namelist, filter, alphasort)) == -1)
    return -1;

  printf("[cgra_open] scanning /sys/class/uio ...\n");
  CGRA_info.dma_mmap = 0;
  CGRA_info.reg_mmap = NULL;
  CGRA_info.reg_mmap_size = 0;
  CGRA_info.ddr_mmap = 0;
  ...
  for (dir = 0; dir < num_dirs; ++dir) {
    ...
    snprintf(path, sizeof(path), "/sys/class/uio/%s/name", namelist[dir]->d_name);
    if ((fpn = fopen(path, "r")) != NULL) {
      if (fgets(uio_name, sizeof(uio_name), fpn) != NULL) strip_trailing_ws(uio_name);
      fclose(fpn);
    }
    snprintf(size_path, sizeof(size_path), "/sys/class/uio/%s/maps/map0/size", namelist[dir]->d_name);
    if ((fps = fopen(size_path, "r")) != NULL) {
      if (fgets(uio_size, sizeof(uio_size), fps) != NULL) strip_trailing_ws(uio_size);
      fclose(fps);
    }
    printf("[cgra_open] %s name='%s' size='%s'\n", namelist[dir]->d_name, uio_name, uio_size);
	...
	continue;
    ...
    else if (is_target_dev(namelist[dir]->d_name, UIO_AXI_CGRA) || is_target_dev(namelist[dir]->d_name, UIO_AXI_GENERIC)) {
      int reg_size_local = get_reg_size(namelist[dir]->d_name);
      const char *dev_name = is_target_dev(namelist[dir]->d_name, UIO_AXI_CGRA) ? UIO_AXI_CGRA : UIO_AXI_GENERIC;
      sprintf(path, "/dev/%s", namelist[dir]->d_name);
      printf("[cgra_open] candidate AXI device %s\n", path);
      if ((fd_reg = open(path, O_RDWR | O_SYNC)) == -1) {
        printf("open failed in AXI CGRA device: %s\n", path);
        free(namelist[dir]);
        return -1;
      }
      if (reg_size_local <= 0) {
        printf("invalid CGRA map size for %s\n", path);
        printf("hint: verify /sys/class/uio/%s/maps/map0/size\n", namelist[dir]->d_name);
        close(fd_reg);
        free(namelist[dir]);
        return -1;
      }
      ...
      CGRA_info.reg_mmap_size = (U64)reg_size_local;
      CGRA_info.reg_mmap = (U32*)mmap(NULL, reg_size_local, PROT_READ|PROT_WRITE, MAP_SHARED, fd_reg, 0);
      close(fd_reg);
      if (CGRA_info.reg_mmap == MAP_FAILED) {
        printf("fd_reg mmap() failed. errno=%d size=0x%x dev=%s\n", errno, reg_size_local, dev_name);
        CGRA_info.reg_mmap = NULL;
        free(namelist[dir]);
        return -1;
      }
      printf("%s: %s mapped size=0x%x\n", path, dev_name, reg_size_local);
  ...
  if (!fd_dma_found) {
    printf("cgra_open(): dma-controller UIO not found or mmap failed\n");
    return -1;
  }
  if (!fd_reg_found) {
    printf("cgra_open(): CGRA UIO not found or mmap failed\n");
    return -1;
  }

  if (CGRA_info.reg_mmap != NULL && CGRA_info.ddr_mmap != 0) {
    printf("[cgra_open] reg_mmap=%p ddr_mmap=%p dma_mmap=%p\n", (void*)CGRA_info.reg_mmap, (void*)CGRA_info.ddr_mmap, (void*)CGRA_info.dma_mmap);
  }
  if (!fd_ddr_found) {
    printf("cgra_open(): ddr_high UIO not found or mmap failed\n");
    return -1;
  }

  if (CGRA_info.reg_mmap == NULL || CGRA_info.dma_mmap == 0 || CGRA_info.ddr_mmap == 0) {
    printf("cgra_open(): one or more mappings are NULL\n");
    return -1;
  }

  if (fd_dma_found) {
    if (CGRA_info.dma_mmap == 0) {
      printf("cgra_open(): dma mapping is NULL\n");
      return -1;
    }
...
void dma_write(U64 DDR_Offset, U64 IP_Offset, U64 size){
	...
	if (cgra.dma_ctrl == 0) {
	  printf("dma_write(): dma_ctrl is not initialized\n");
	  return;
	}
	...
	*(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD0) = LMM_BASE_PHYS + IP_Offset;
	...
	do {
	  ...
	  if (--timeout == 0) {
	    printf("dma_write(): timeout waiting ZDMA_CH_STATUS\n");
	    return;
	  }
